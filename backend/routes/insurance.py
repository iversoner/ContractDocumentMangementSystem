...
def _row_to_dict(r) -> dict:
    ...
    if 'source' in r.keys() and r['source'] == 'scan':
        is_complete = bool(
            r['plate_no'] and r['brand'] and r['insurance_company'] and r['insurance_type'] and r['agent']
...
def list_insurances():
    ...
    rows = db.execute(
        f"SELECT * FROM insurances WHERE {where_sql} ORDER BY {order_col} {order_dir} LIMIT ? OFFSET ?",
