...
- 支持合同、专利、车险三种模块
- **逐级浏览子目录**：非递归扫描，显示当前目录的文件和子目录列表，用户可点击子目录进入下级
- **面包屑导航**：显示当前路径，点击路径段可快速跳转
- **返回上级**：支持一键返回上级目录
- **选择性导入**：扫描结果每行带复选框，支持全选/取消全选，用户可选择部分文件导入（非强制全选）
- 文件路径去重，避免重复录入
- **Windows 路径自动翻译**：用户粘贴 Windows 本地路径（如 `E:\Data\contract`），后端自动翻译为容器内 Linux 路径（`/data/contract`）。路径必须位于配置的宿主机数据目录下，否则拒绝并提示
- 配置通过 `HOST_DATA_DIR` 环境变量传入，setup.bat 自动设置
- 已存在文件折叠显示，避免干扰
